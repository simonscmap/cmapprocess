...
metadata = pd.read_html(data_base_path + "AMT_Flow_Cytometry/metadata/BODC_sample_metadata_report.html")
flag_data = pd.read_html(data_base_path + "AMT_Flow_Cytometry/metadata/FlagData.html")[1]
flag_data.columns = flag_data.columns.droplevel(0)
df = pd.read_csv(data_base_path + "AMT_Flow_Cytometry/data/botlist.csv",sep=",")
df["yyyy-mm-ddThh24:mi:ss[GMT]"] = pd.to_datetime(df["yyyy-mm-ddThh24:mi:ss[GMT]"],format='%d/%m/%Y %H:%M')
...
#  'prymnesiophyceae_abundance_P490A00Z',
 "synechococcusceae_abundance_P490A00Z_flag",#QV:BODC.10
    ...
    df.drop(columns=["ODV_type","site","bottle_reference",
    ...
    "nanoeukaryotic_abundance_X726A86B_flag"],inplace=True)
    #reorder columns
    df = df[[ 'time', 'lat', 'lon', 'depth', 'synechococcus_carbon_biomass_C700A90Z', 'prochlorococcus_carbon_biomass_C701A90Z', 'bacteria_abundance_C804B6A6', 'picoeukaryotic_carbon_biomass_CYEUA00A', 'bacteria_abundance_std_dev_H396080A', 'bacteria_carbon_biomass_HBBMAFTX', 'bacteria_abundance_HBCCAFTX_Tarran', 'bacteria_abundance_HBCCAFTX_Zubkov', 'cryptophyceae_abundance_J79A0596', 'bacteria_abundance_P18318A9', 'prymnesiophyceae_abundance_P490A00Z', 'synechococcus_abundance_P700A90Z_Tarran', 'synechococcus_abundance_P700A90Z_Zubkov', 'prochlorococcus_abundance_P701A90Z_Tarran', 'prochlorococcus_abundance_P701A90Z_Zubkov', 'picoeukaryotic_abundance_PYEUA00A_Tarran', 'picoeukaryotic_abundance_PYEUA00A_Zubkov', 'bacteria_abundance_TBCCAFTX_Tarran', 'bacteria_abundance_TBCCAFTX_Zubkov', 'nanoeukaryotic_abundance_X726A86B', 'bodc_station', 'original_station', 'gear', 'BODC_bottle_code', 'bottle_flag', 'rosette_position', 'firing_sequence', 'w_depth', 'bottle_pressure', 'cruise']]
...
def compile_AMT_cruise(cleaned_df,dataset_meta_data_df, vars_meta_data_df):
    for cruise in cleaned_df['cruise'].unique():
            ...
            amt_core_keywords = """cruise,observation,insitu,in-situ,flow cytometry, flow cytometer, AMT, Atlantic Meridional Transect,biology,biogeochemistry,{cruise_name},{cruise_nickname}""".format(cruise_name =cruise_details['Name'].iloc[0],cruise_nickname = cruise_details['Nickname'].iloc[0])
            ...
            'bottle_flag': ['Bottle Flag','Uncategorized','','irregular','irregular','Biology+BioGeoChemistry+Biogeography','0',amt_core_keywords,str(flag_data)],
            ...
            'bottle_pressure': ['Bottle Pressure','Uncategorized','','irregular','irregular','Biology+BioGeoChemistry+Biogeography','0',amt_core_keywords,''],
            }
            ...
            dataset_meta_data_df["dataset_distributor"] = ["British Oceanography Data Centre - National Oceanography Centre"]
            dataset_meta_data_df["dataset_acknowledgement"] = ["Contains data supplied by Natural Environment Research Council."]
            dataset_meta_data_df["dataset_history"] = [""]
            dataset_meta_data_df["dataset_description"] = ["""This dataset contains flow-cytometry data from the {name} {nickname} research cruise. Data was provided by the British Oceanography Data Centre (BODC).""".format(name=cruise_details['Name'].iloc[0],nickname=cruise_details['Nickname'].iloc[0])]
            ...
            vars_meta = pd.DataFrame.from_dict(overlap_dict, orient='index').reset_index().rename(columns={'index':'var_short_name'})
            vars_meta.columns = list(vars_meta_data_df)

            combine_df_to_excel(vs.staging+'combined/'+cruise+'_flow_cytometry.xlsx',cruise_data,dataset_meta_data_df, vars_meta)
