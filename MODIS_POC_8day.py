...
for fil in tqdm(files_in_range):
    ...
    DB.toSQLbcp(vs.satellite + 'tblModis_POC/rep/' +fil.strip('.nc') + '.csv', 'tblModis_POC', 'Rainier')
    stats.buildLarge_Stats(df, df.time.iloc[0]+'_8_day', 'tblModis_POC', 'satellite', transfer_flag="normal")
    xdf.close()
