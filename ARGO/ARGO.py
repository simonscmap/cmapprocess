...
#dev note as of march 5th EOD - test on real internet
...
def process_chunked_df(df):
    # This functions cleans chunked df
    df = slow_gc_data_decode(df)
    # df = data.decode_df_columns(df)
    """downcasts numeric cols"""
...
def format_split_bgc(syn_float, bgc_template_df):
    ...
    # """xarray to csv"""
    xarray_to_export_csv(fil)
    # """load netcdf into dataframe"""
    df = load_xarrayed_csv()
    chunk_list = []
    for chunk in df():
        cdf = process_chunked_df(chunk)
    chunk_list.append(cdf)
    df_concat = pd.concat(chunk_list)

    """calls func that returns DataFrame of metadata"""
    bgc_metadata_df = format_bgc_metadata(df_concat,bgc_metadata_columns,float_id)
