...
def fill_depth_if_missing(df,depth_replace_col):
    ...
    if depth_replace_col in list(df):
        if 'depth' not in list(df):
            all_depth_neg = all(df[depth_replace_col]< 0) #all values are above sea surface..
            if all_depth_neg == False: #some vals may be outliers, but will be used for depth
...
def extract_edmo_cruise_metadata(df):
    ...
    edmo_cruise_metadata_list = [cruise_name, cruise_type, EDMO_code]
...
def shared_AMT_data_processing(df,meta_html, depth_replace_col):
    ...
    cleaned_df, edmo_cruise_metadata_list =clean_AMT_specific_cols(df,depth_replace_col)
...
def process_amt_01_bathemetry():
    ...
    combine_df_to_excel(vs.staging + 'combined/' + 'tblJR19950921_AMT01_bathemetry_qa.xlsx',cleaned_df,dataset_meta, vars_meta)
...
def process_amt_01_meterology():
    ...
    cleaned_df, dataset_meta, vars_meta = shared_AMT_data_processing(df,meta_html,"depth_below_surf")
    combine_df_to_excel(vs.staging + 'combined/' + 'tblJR19950921_AMT01_meterology_qa.xlsx',cleaned_df,dataset_meta, vars_meta)
...
def process_amt_01_tsg():
    ...
    cleaned_df, dataset_meta, vars_meta = shared_AMT_data_processing(df,meta_html,"depth_below_surf")
    combine_df_to_excel(vs.staging + 'combined/' + 'tblJR19950921_AMT01_TSG_qa.xlsx',cleaned_df,dataset_meta, vars_meta)
...
def process_amt_01_CTD():
    ...
    combine_df_to_excel(vs.staging + 'combined/' + 'tblJR19950921_AMT01_CTD_qa.xlsx',cleaned_df,dataset_meta, vars_meta)
...
def process_amt_01_Nutrient():
    ...
    df_concat = pd.concat(new).fillna(method='ffill')
    cleaned_df, dataset_meta, vars_meta = shared_AMT_data_processing(df_concat,meta_html,"depth_below_surf")
    combine_df_to_excel(vs.staging + 'combined/' + 'tblJR19950921_AMT01_Nutrient_qa.xlsx',cleaned_df,dataset_meta, vars_meta)
...
def process_amt_02_nutrient_depth_profiles():
    meta_html = pd.read_html(data_base_path + "AMT02/Nutrient/depth_profiles/1093485.html")
...
def process_amt_04_extracted_chlorophyll_profiles():
    meta_html = pd.read_html(data_base_path + "AMT04/Extracted_Chlorophyll/1870667.html")
